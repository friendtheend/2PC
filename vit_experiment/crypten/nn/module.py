...
class Graph(Container):
    ...
    def forward(self, *args):
        ...
        while node_to_compute is not None:
            ...
            elif (isinstance(module, (Sigmoid, SiLU))):
...
def _align_plain_for_encrypted(enc, plain):
    if os.environ.get("CRYPTEN_PROFILE_ALIGN", "0") != "1":
        return plain
    ...
    try:
        ...
        return plain
    if len(enc_shape) != len(plain_shape):
        return plain
    ...
    out = plain
    for dim, (es, ps) in enumerate(zip(enc_shape, plain_shape)):
        if ps == es or ps == 1:
            continue
        if ps > 0 and es % ps == 0:
            factor = es // ps
