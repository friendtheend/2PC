...
class Graph(Container):
    ...
    def __init__(self, input_names, output_names, modules=None, graph=None):
        ...
        self.silu_comm_bytes = 0
        self.silu_comm_time = 0
    ...
    def forward(self, *args):
        ...
        def _mark_as_computed(name):
            ...
            computed[name] = True
            for key, value_list in self._graph.items():
                for idx, dep_name in enumerate(value_list):
                    if dep_name == name:
                        inputs_available[key][idx] = True
        ...
        while node_to_compute is not None:
            ...
            elif (isinstance(module, (GELU, Sigmoid, SiLU))):
                self.silu_time += module_time
                self.silu_comm_bytes += comm_bytes
                self.silu_comm_time += comm_time
                self.silu_comm_rounds += comm_rounds
            elif isinstance(module, Mul):
                # SiLU in ONNX is typically decomposed to Sigmoid + Mul.
                # Count this Mul as SiLU-related when one input comes from a Sigmoid node.
                input_names = self._graph.get(node_to_compute, [])
                is_silu_mul = any(
                    name in self._modules and isinstance(self._modules[name], Sigmoid)
            ...
            if all(computed[output_name] for output_name in self.output_names):
                ...
                if report_cost:
                    ...
                    print(f"silu_time: {self.silu_time} sec")
                    ...
                    print(f"silu_comm_time: {self.silu_comm_time} sec")
                    ...
                    print(f"silu_comm_bytes: {self.silu_comm_bytes / 1073741824} GB")
                    ...
                    print(f"silu_comm_rounds: {self.silu_comm_rounds}")
        ...
        # Best-effort fallback for profiling runs where exported graphs may have
        # dangling outputs after ONNX lowering differences. We still return
        # accumulated cost stats instead of aborting.
        if len(values) > 0:
            if crypten.comm.get().get_rank() == 0:
                print("WARNING: graph output unresolved; returning best-effort profiling result.")
                if os.environ.get("CRYPTEN_DEBUG_GRAPH", "0") == "1":
                    limit = int(os.environ.get("CRYPTEN_DEBUG_GRAPH_LIMIT", "20"))
                    pending = []
                    for key, avail in inputs_available.items():
                        if computed.get(key, False):
                            continue
                        deps = self._graph.get(key, [])
                        missing = [deps[i] for i, ok in enumerate(avail) if not ok]
                        pending.append((len(missing), key, missing))
                    pending.sort(key=lambda x: x[0])
                    print("----- DEBUG_GRAPH pending nodes (fewest missing first) -----")
                    for miss_count, key, missing in pending[:limit]:
                        dep_dbg = []
                        for dep in missing[:6]:
                            dep_dbg.append(
                                f"{dep}(exists={dep in computed},done={computed.get(dep, None)},in_values={dep in values})"
                            )
                        print(f"PENDING node={key} missing={miss_count} deps={dep_dbg}")
                    forced_name = os.environ.get("CRYPTEN_PROFILE_OUTPUT_NAME", "").strip()
                    if forced_name:
                        target = next((x for x in pending if x[1] == forced_name), None)
                        if target is None:
                            print(f"DEBUG target_output={forced_name} already computed or absent in graph.")
                        else:
                            print(
                                f"DEBUG target_output={forced_name} missing_deps={target[2][:10]}"
                            )
                if cfg.debug.report_cost:
                    print("====================================================")
                    print(f"total_running_time: {time.time() - forward_start_time} sec")
                    print(f"embedding_time: {self.embedding_time} sec")
                    print(f"matmul_time: {self.matmul_time} sec")
                    ...
                    print(f"other_comm_rounds: {self.other_comm_rounds}")
                    print("====================================================")
                print(f"running time: {time.time() - forward_start_time:.2f}s, comm byte: {self.total_comm_bytes / 1073741824:.2f} GB")
            return next(reversed(values.values()))
        raise ValueError("nn.Graph.forward() failed. Is graph unconnected and produced no intermediate values?")
...
def _sanitize_binary_tensor(x):
    if torch.is_tensor(x) and torch.is_complex(x):
        return x.real
    return x


def _align_plain_for_encrypted(enc, plain):
    if not torch.is_tensor(plain):
        return plain

    # Keep plain tensors on the same device as encrypted tensors.
    try:
        enc_device = enc.device
        if plain.device != enc_device:
            plain = plain.to(enc_device)
    except Exception:
        pass

    try:
        enc_shape = tuple(enc.size())
        plain_shape = tuple(plain.size())
    except Exception:
        return plain

    # If rank differs, prepend singleton dimensions to enable standard
    # broadcasting and optional profile-time expansion below.
    if len(plain_shape) < len(enc_shape):
        new_shape = [1] * (len(enc_shape) - len(plain_shape)) + list(plain_shape)
        plain = plain.reshape(new_shape)
        plain_shape = tuple(plain.size())
    elif len(plain_shape) != len(enc_shape):
        return plain

    # Keep normal broadcasting behavior if already compatible.
    compatible = True
    for es, ps in zip(enc_shape, plain_shape):
        if ps == 1 or ps == es:
            continue
        compatible = False
        break
    ...
    # Guard against accidental broadcast growth on encrypted batch dims.
    # Example: plain shape (2, ...) with encrypted shape (1, ...) can
    # broadcast to batch 2 and later break downstream reshapes.
    out = plain
    out_shape = tuple(out.size())
    for dim, (es, ps) in enumerate(zip(enc_shape, out_shape)):
        if es == ps:
            continue
        if es == 1 and ps > 1:
            out = out.narrow(dim, 0, 1)

    # Profiling-focused fallback for RoPE-like shape patterns:
    # if one dim is an integer divisor of encrypted dim, expand by
    # repeat_interleave to match exactly.
    out_shape = tuple(out.size())
    for dim, (es, ps) in enumerate(zip(enc_shape, out_shape)):
        if ps == es or ps == 1:
            continue
...
def _lenient_binary_fallback(x, y, op_name, exc):
    if os.environ.get("CRYPTEN_PROFILE_LENIENT", "0") != "1":
        raise exc

    if crypten.is_encrypted_tensor(x):
        fallback = x
    elif crypten.is_encrypted_tensor(y):
        fallback = y
    ...
    if crypten.comm.get().get_rank() == 0:
        print(
            f"WARNING: lenient fallback for {op_name}: {type(exc).__name__}: {exc}"
        )
    return fallback
...
class Add(Module):
    ...
    def forward(self, input):
        ...
        x, y = _sanitize_binary_tensor(input[0]), _sanitize_binary_tensor(input[1])
        if torch.is_tensor(x) and torch.is_tensor(y) and x.device != y.device:
            y = y.to(x.device)
        try:
            if crypten.is_encrypted_tensor(x) is not True and crypten.is_encrypted_tensor(y):
                x = _align_plain_for_encrypted(y, x)
                return y.add(x)
            if crypten.is_encrypted_tensor(x) and torch.is_tensor(y):
                y = _align_plain_for_encrypted(x, y)
            return x.add(y)
        except Exception as exc:
            return _lenient_binary_fallback(x, y, "Add", exc)
...
class Sub(Module):
    ...
    def forward(self, input):
        ...
        try:
            ...
            return _lenient_binary_fallback(x, y, "Sub", exc)
...
class Mul(Module):
    ...
    def forward(self, input):
        ...
        try:
            ...
            if os.environ.get("CRYPTEN_DEBUG_SHAPES", "0") == "1" and crypten.comm.get().get_rank() == 0:
                x_shape = tuple(x.size()) if hasattr(x, "size") else "n/a"
                y_shape = tuple(y.size()) if hasattr(y, "size") else "n/a"
                x_dev = str(x.device) if hasattr(x, "device") else "n/a"
                y_dev = str(y.device) if hasattr(y, "device") else "n/a"
                print(
                    f"DEBUG Mul node={getattr(self, '_graph_name', 'n/a')} "
                    f"x_shape={x_shape} y_shape={y_shape} x_dev={x_dev} y_dev={y_dev}"
                )
            return _lenient_binary_fallback(x, y, "Mul", exc)
        ...
        try:
            if crypten.is_encrypted_tensor(x):
                if torch.is_tensor(y):
                    y = _align_plain_for_encrypted(x, y)
                    y = crypten.cryptensor(y, device=x.device)
                return x / y
            if crypten.is_encrypted_tensor(y):
                if torch.is_tensor(x):
                    x = _align_plain_for_encrypted(y, x)
                    x = crypten.cryptensor(x, device=y.device)
                return x / y
            return x / y
        except Exception as exc:
            return _lenient_binary_fallback(x, y, "Div", exc)
...
class Neg(Module):
    """Module that negates the input tensor."""
...
class Cos(Module):
    """Module that applies cosine element-wise."""
...
class Sin(Module):
    """Module that applies sine element-wise."""
...
class Tile(Module):
    """
    Module that tiles an input tensor according to repeat factors.
    ONNX input format: [tensor, repeats].
    """

    SUPPORTS_PLAINTEXT_INPUTS = True

    def forward(self, x):
        tensor, repeats = tuple(x)
        if crypten.is_encrypted_tensor(repeats):
            raise ValueError("Tile does not support encrypted repeats.")

        if torch.is_tensor(repeats):
            repeats = repeats.long().tolist()
        ...
        if len(repeats) < tensor_dim:
            repeats = [1] * (tensor_dim - len(repeats)) + repeats
        elif len(repeats) > tensor_dim:
            new_shape = [1] * (len(repeats) - tensor_dim) + list(tensor.size())
...
class Trilu(Module):
    """
    Module that returns the upper or lower triangular part of a matrix.
    ONNX input format: [input] or [input, k].
    """
    ...
    def _parse_diagonal(self, diagonal):
        if crypten.is_encrypted_tensor(diagonal):
            raise ValueError("Trilu does not support encrypted diagonal input.")
        if torch.is_tensor(diagonal):
            if diagonal.numel() != 1:
                raise ValueError("Trilu diagonal input must be a scalar.")
            return int(diagonal.item())
        return int(diagonal)
    ...
    def forward(self, x):
        ...
        if isinstance(x, (list, tuple)):
            if len(x) == 1:
                ...
                x, diagonal = tuple(x)
                diagonal = self._parse_diagonal(diagonal)
            else:
                raise ValueError(
                    f"Trilu expects 1 or 2 inputs, but received {len(x)}."
                )

        # ONNX Trilu is defined for rank >= 2. Be permissive for odd exports.
        if x.dim() < 2:
            return x

        rows, cols = x.size(-2), x.size(-1)
        row_idx = torch.arange(rows, device=x.device).view(rows, 1)
        col_idx = torch.arange(cols, device=x.device).view(1, cols)
        if self.upper:
            mask2d = (col_idx - row_idx) >= diagonal
        else:
            mask2d = (col_idx - row_idx) <= diagonal

        mask = mask2d.view([1] * (x.dim() - 2) + [rows, cols]).expand(list(x.size()))

        # CrypTensor expects arithmetic mask values.
        if crypten.is_encrypted_tensor(x):
            return x * mask.float()
    ...
    def from_onnx(attributes=None):
        if attributes is None:
            attributes = {}
        return Trilu(upper=attributes.get("upper", 1))
...
class Reshape(Module):
    ...
    def forward(self, tensor, shape=None):
        ...
        try:
            ...
            if os.environ.get("CRYPTEN_DEBUG_SHAPES", "0") == "1" and crypten.comm.get().get_rank() == 0:
                tensor_shape = tuple(tensor.size()) if hasattr(tensor, "size") else "n/a"
                print(
                    f"DEBUG Reshape node={getattr(self, '_graph_name', 'n/a')} "
                    f"tensor_shape={tensor_shape} target_shape={tuple(shape)}"
                )
            if os.environ.get("CRYPTEN_PROFILE_LENIENT", "0") != "1":
                raise
            if crypten.comm.get().get_rank() == 0:
                print(
                    f"WARNING: lenient fallback for Reshape: {type(exc).__name__}: {exc}"
                )
            return tensor
...
class MatMul(Module):
    ...
    def forward(self, x):
        if hasattr(self, "weight"):
            weight = self.weight
            if crypten.is_encrypted_tensor(x) and torch.is_tensor(weight):
                weight = crypten.cryptensor(weight, device=x.device)
            elif torch.is_tensor(x) and crypten.is_encrypted_tensor(weight):
                x = crypten.cryptensor(x, device=weight.device)
            if torch.is_tensor(x) and not torch.is_floating_point(x):
                x = x.float()
            if torch.is_tensor(weight) and not torch.is_floating_point(weight):
                weight = weight.float()
            ...
            lhs, rhs = x[0], x[1]
            if crypten.is_encrypted_tensor(lhs) and torch.is_tensor(rhs):
                rhs = crypten.cryptensor(rhs, device=lhs.device)
            elif torch.is_tensor(lhs) and crypten.is_encrypted_tensor(rhs):
                lhs = crypten.cryptensor(lhs, device=rhs.device)
            elif torch.is_tensor(lhs) and torch.is_tensor(rhs) and lhs.device != rhs.device:
                rhs = rhs.to(lhs.device)
            if torch.is_tensor(lhs) and not torch.is_floating_point(lhs):
                lhs = lhs.float()
            if torch.is_tensor(rhs) and not torch.is_floating_point(rhs):
                rhs = rhs.float()
...
class SiLU(Module):
    r"""
    Module that computes SiLU activations element-wise.
    ...
    def forward(self, x):
        # CrypTensor path uses crypten.common.functions.approximations.silu
        # and therefore honors cfg.functions.silu_method.
        if hasattr(x, "silu"):
            return x.silu()
