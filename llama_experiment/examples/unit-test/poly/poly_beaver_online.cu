...
// Build:
/*
  nvcc -O3 -std=c++17 -arch=sm_86 poly_beaver_online.cu \
       -I"${CONDA_PREFIX}/include" \
       -L"${CONDA_PREFIX}/lib" -lmpi \
       -Xlinker --allow-shlib-undefined \
       -Xlinker -rpath -Xlinker "${CONDA_PREFIX}/lib" \
       -o poly_beaver_online
*/
//
// Run:
//   mpirun -np 2 ./poly_beaver_online --rows 589 --cols 3072 --iters 10
//   mpirun -np 2 ./poly_beaver_online --rows 4096 --cols 11008 --iters 10
...
__global__ void k_add_mod(u64* __restrict__ y,
...
__global__ void k_mul_mod(const u64* __restrict__ x,
  ...
  size_t stride = gridDim.x * blockDim.x;
  for (size_t i = tid; i < n; i += stride) {
    __uint128_t p = ( __uint128_t )x[i] * ( __uint128_t )y[i];
    out[i] = (u64)p & mask;
  }
}

// Beaver elemwise multiply combine:
// given shares: C (share of A*B), A, B
// open values: d_open = x - A (opened), e_open = x - B (opened)
// output share z = x*x (share)
//
...
__global__ void k_poly_eval(const u64* __restrict__ x_share,
  ...
  for (size_t i = tid; i < n; i += stride) {
    // a*x2
    __uint128_t p1 = ( __uint128_t )a * ( __uint128_t )x2_share[i];
    u64 y = (u64)p1 & mask;

    // + b*x
    __uint128_t p2 = ( __uint128_t )b * ( __uint128_t )x_share[i];
    y = (y + ((u64)p2 & mask)) & mask;
...
int main(int argc, char** argv) {
  ...
  // bytes/rounds accounting per iteration (approx for 2-party)
  // Two opens: d and e. For MPI_Allreduce with 2 parties:
  //   each allreduce: send n*8, recv n*8  => 2*n*8 bytes
  ...
  for (int it = 0; it < g_iters; it++) {
    ...
    k_beaver_combine_elemmul<<<blocks, threads>>>(
        d_C, d_A, d_B, d_d_open, d_e_open, d_x2, n, (rank == 0), g_mask);

    // (6) Poly eval: y = a*x2 + b*x + c
    k_poly_eval<<<blocks, threads>>>(d_x, d_x2, d_y, n, a, b, c, g_mask);
  ...
  if (rank == 0) {
    ...
    // (optional) if you still want internal breakdown for analysis, uncomment:
