...
struct OnlineCommStats {
    uint64_t sent_bytes = 0;
    uint64_t recv_bytes = 0;

    void add_allreduce_payload(size_t elem_count) {
        uint64_t bytes = (uint64_t)elem_count * sizeof(u64);
        sent_bytes += bytes;
        recv_bytes += bytes;
    }
};
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        double sent_mb_total = (double)comm_stats.sent_bytes / 1e6;
        double recv_mb_total = (double)comm_stats.recv_bytes / 1e6;
        double sent_mb_iter = sent_mb_total / iters;
        double recv_mb_iter = recv_mb_total / iters;
        double total_mb_iter = sent_mb_iter + recv_mb_iter;
        ...
        printf("║  Comm bytes:  %8.2f / %8.2f MB/iter     ║\n",
               sent_mb_iter, recv_mb_iter);
        printf("║                                              ║\n");
        ...
        printf("╚══════════════════════════════════════════════════╝\n\n");
        printf("[Party 0] Communication per iter: Sent=%.6f MB, Recv=%.6f MB, Total=%.6f MB\n",
               sent_mb_iter, recv_mb_iter, total_mb_iter);
        printf("[Party 0] Communication total: Sent=%.6f MB, Recv=%.6f MB, Total=%.6f MB\n",
               sent_mb_total, recv_mb_total, sent_mb_total + recv_mb_total);
    }
