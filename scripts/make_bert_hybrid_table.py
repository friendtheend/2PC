#!/usr/bin/env python3
"""Generate a BERT-base SHAFT vs BriLLMFlow online comparison table."""
...
DISPLAY = {
    "bert_W_QKV": r"W\_Q/K/V",
    "bert_W_O": r"W\_O",
    "bert_W_1": r"W\_1",
    "bert_W_2": r"W\_2",
    "bert_QKT": r"Q@K$^T$",
    "bert_scoresV": r"scores@V",
}
...
def load_brillm(path: Path) -> Dict[str, Dict[str, float]]:
    ...
    with path.open(newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            label = row["label"]
            if label not in BERT_COUNTS:
                continue

            count = int(float(row.get("count") or BERT_COUNTS[label]))
...
def main() -> int:
    ...
    parser.add_argument("--model-title", default="BERT-base, 12 layers")
    ...
    (args.out_dir / "bert_hybrid_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")

    with (args.out_dir / "bert_hybrid_summary.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(summary.keys()))
        writer.writeheader()
        writer.writerow(summary)

    with (args.out_dir / "bert_brillm_perop.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    latex = [
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{BERT-base online phase comparison with SHAFT (QNLI, seq=128, 12 layers). SHAFT uses Fourier GELU; BriLLMFlow replaces linear operators with GPU integer GEMM and uses MixPoly GELU.}",
        r"\label{tab:bert_shaft_comparison}",
    ...
    tex = "\n".join(latex)
    (args.out_dir / "bert_table.tex").write_text(tex, encoding="utf-8")
