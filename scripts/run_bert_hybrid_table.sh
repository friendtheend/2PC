...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<'EOF'
Run a BERT-base QNLI seq=128 SHAFT vs BriLLMFlow online comparison and
generate a paper-style Table V.

Usage:
  bash scripts/run_bert_hybrid_table.sh

Common environment overrides:
  OUT=results/bert_hybrid_test
  SHAFT_ROOT=/path/to/shaft/tree        # default: ./vit_experiment
  TASK_NAME=qnli
  MODEL_NAME=andeskyl/bert-base-cased-qnli
  LEN_DATA=128
  MAX_LENGTH=128
  BATCH_SIZE=1
...
Outputs:
  $OUT/shaft_bert_qnli_l128_comp.log
  $OUT/shaft_bert_qnli_l128_report.log
  $OUT/brillm_online_bert_*.log
  $OUT/bert_brillm_raw_online.csv
  $OUT/bert_brillm_perop.csv
  $OUT/bert_hybrid_summary.csv
  $OUT/bert_table.tex
  $OUT/table5.tex
EOF
  exit 0
...
OUT="${OUT:-${ROOT}/results/bert_hybrid_$(date +%Y%m%d_%H%M%S)}"
BIN="${BIN:-${ROOT}/brillmflow_2pc/BMT/build/gpu_matrix_beaver}"
TASK_NAME="${TASK_NAME:-qnli}"
MODEL_NAME="${MODEL_NAME:-andeskyl/bert-base-cased-${TASK_NAME}}"
...
SHAFT_LOG="$OUT/shaft_bert_${TASK_NAME}_l${LEN_DATA}_report.log"
if [[ "$RUN_SHAFT" == "1" ]]; then
  cd "$SHAFT_ROOT/examples/text-classification"
  if [[ "$RUN_SHAFT_COMP" == "1" ]]; then
    SHAFT_COMP_ARGS=(
      ...
      --output_dir "$OUT/eval_${TASK_NAME}_comp"
      --gelu_method "$GELU_METHOD"
    )
    printf '%q ' "${SHAFT_COMP_ARGS[@]}" > "$OUT/shaft_comp_command.txt"
    echo >> "$OUT/shaft_comp_command.txt"
    "${SHAFT_COMP_ARGS[@]}" 2>&1 | tee "$OUT/shaft_bert_${TASK_NAME}_l${LEN_DATA}_comp.log"
  fi
  ...
  SHAFT_ARGS=(
    ...
    --max_length "$MAX_LENGTH"
    --per_device_eval_batch_size "$BATCH_SIZE"
    --output_dir "$OUT/eval_${TASK_NAME}_report"
    --estimate_mode total
    --report_cost
    --gelu_method "$GELU_METHOD"
  )
...
if [[ "$RUN_BRILLM" == "1" ]]; then
  ...
  run_one bert_W_QKV    128 768  768  1000
  run_one bert_W_O      128 768  768  1000
  run_one bert_W_1      128 768  3072 500
  run_one bert_W_2      128 3072 768  500
  run_one bert_QKT      128 64   128  1000
  run_one bert_scoresV  128 128  64   1000
fi

python "$ROOT/scripts/parse_brillm_logs.py" \
  --kind online \
  --out "$OUT/bert_brillm_raw_online.csv" \
  bert_W_QKV="$OUT/brillm_online_bert_W_QKV.log" \
  bert_W_O="$OUT/brillm_online_bert_W_O.log" \
  bert_W_1="$OUT/brillm_online_bert_W_1.log" \
  bert_W_2="$OUT/brillm_online_bert_W_2.log" \
  bert_QKT="$OUT/brillm_online_bert_QKT.log" \
  bert_scoresV="$OUT/brillm_online_bert_scoresV.log"

python "$ROOT/scripts/make_bert_hybrid_table.py" \
  --shaft-log "$SHAFT_LOG" \
  --brillm-csv "$OUT/bert_brillm_raw_online.csv" \
  --out-dir "$OUT"

echo "OUT=$OUT"
echo "LaTeX: $OUT/table5.tex"
echo "Summary: $OUT/bert_hybrid_summary.csv"
